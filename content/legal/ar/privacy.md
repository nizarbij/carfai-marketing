...
المعالجون المستقبليون مدرجون في ملحق خارطة الطريق أدناه من أجل الشفافية. سنقوم بإخطارك (عبر البريد الإلكتروني ولافتة داخل التطبيق) قبل 30 يومًا على الأقل من إضافة أي معالج فرعي يعالج بياناتك الشخصية، مع إتاحة الفرصة لإنهاء حسابك قبل أن يدخل التغيير حيز التنفيذ.
...
2. **معايير المجتمع المجهولة** — إحصائيات مجمعة عبر جميع مستخدمي CarFai الذين لديهم نفس الصانع/الطراز/السنة/المنطقة. أمثلة: متوسط سعر الشراء، متوسط تكلفة التأمين، الفاصل الزمني المعتاد للصيانة. يتم استردادها عبر دوال SQL من نوع SECURITY DEFINER (`get_carfai_market_data`، `get_carfai_financial_benchmark`) التي **ترجع التجميعات فقط** — لا ترجع أبدًا صفوفًا فردية أو أسعارًا أو معرفات مستخدمين. **معيار إخفاء الهوية**: [JURISDICTION-SPECIFIC: سيتم التحديد — سيتم تحديده وفقًا للمادة 28 من Quebec Law 25؛ عنصر نائب في انتظار السؤال 5 في `docs/legal/REVIEW_FINDINGS_2026-05-09.md`. من المحتمل أن يكون عدم الكشف عن الهوية k-anonymity مع حد أدنى لحجم مجموعة التجميع.] لا يمكن إعادة تحديد هويتك من استجابة المعيار.

3. **محتوى البحث على الويب** (عندما يكون ذا صلة) — للسياق الحساس للوقت (مثل أسعار الوقود الحالية، عمليات الاستدعاء الأخيرة، تحولات السوق)، قد يسترد AI Advisor عددًا صغيرًا من صفحات الويب العامة عبر [سيتم التحديد — خدمة البحث وفقًا للسؤال 8]. يتم التعامل مع المحتوى الذي تم جلبه كمدخل غير موثوق به (وفقًا لدفاعات حقن الأوامر في القسم L.2) ولا يتم كتابته أبدًا في حساب CarFai الخاص بك.
...
[JURISDICTION-SPECIFIC: سيتم التحديد — القسم في انتظار قرار المؤسس بشأن السؤال 4. تقييم أداء المديرين هو مفهوم ميزة B2B أظهرته المراجعة القانونية. إشعار القرار المؤتمت بموجب المادة 12.1 من Quebec Law 25 + مواصفات الخوارزمية (5 فئات مرجحة، المدخلات، نطاق المخرجات، من يرى ماذا) سيتم إدراجه كجزء من القسم M1.3 من `RELEASE_PLAN.md` في Sprint 4. حتى ذلك الحين، تقييم أداء المديرين ليس في مرحلة الإنتاج. سيتم توسيع هذا القسم بمواصفات الخوارزمية عند إطلاق الميزة.]
...
**[POST-LAUNCH]** — عندما نوفر بنية تحتية إقليمية مخصصة في الاتحاد الأوروبي، سنقوم بتحديث هذه السياسة وتوجيه مستخدمي المنطقة الاقتصادية الأوروبية وفقًا لذلك.
...
لا يستخدم تطبيق CarFai للهاتف المحمول ملفات تعريف الارتباط. يستخدم موقع CarFai التسويقي (`carfai.app` وأي نطاقات فرعية) ملفات تعريف الارتباط الأساسية لتشغيل الموقع. انظر سياسة ملفات تعريف الارتباط المنفصلة على `https://carfai.app/cookies` (بعد الإطلاق).
...
يتم أرشفة الإصدارات السابقة في مستودع المستندات القانونية العام الخاص بنا (سيتم تحديد الرابط بعد الإطلاق).

## ملحق خارطة الطريق — المعالجون المستقبليون (للمعلومات)

مدرجة للشفافية. لا تعالج حاليًا بياناتك الشخصية. ستتم إضافتها إلى القسم 3 (وسيتم إخطارك وفقًا للسياسة أعلاه) عند بدء التكامل.

| المعالج | الخدمة | الهدف | رابط الخصوصية |
|---|---|---|---|
| Sentry | الإبلاغ عن الأعطال والأخطاء | Sprint 4 (وفقًا لـ `RELEASE_PLAN.md`) | https://sentry.io/privacy/ |
| PostHog | تحليلات المنتج | Sprint 4 | https://posthog.com/privacy |
| Resend | البريد الإلكتروني للمعاملات (الترحيب، الفاتورة، إعادة تعيين كلمة المرور) | Sprint 4 H2.0 | https://resend.com/legal/privacy-policy |
...
- **ممثل المنطقة الاقتصادية الأوروبية (إذا كان مطلوبًا بموجب المادة 27 من GDPR)**: `[سيتم تحديده بواسطة المحامي]`
- **ممثل المملكة المتحدة (إذا كان مطلوبًا بموجب المادة 27 من UK GDPR)**: `[سيتم تحديده بواسطة المحامي]`
