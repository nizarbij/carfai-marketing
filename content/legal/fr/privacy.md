...
2.  **Indicateurs de référence communautaires anonymes** — statistiques agrégées sur tous les utilisateurs de CarFai ayant la même marque/modèle/année/région. Exemples : prix d'achat médian, coût moyen de l'assurance, intervalle d'entretien typique. Récupérés via des fonctions SQL SECURITY DEFINER (`get_carfai_market_data`, `get_carfai_financial_benchmark`) qui **ne retournent que des agrégats** — jamais de lignes individuelles, de prix ou d'identifiants d'utilisateur. **Norme d'anonymisation** : les fonctions d'agrégation renvoient des statistiques regroupées au niveau de la cohorte marque / modèle / année / région uniquement ; les enregistrements individuels d'utilisateurs, les prix ou les identifiants ne sont jamais retournés dans les réponses de l'API. Nous appliquons des seuils minimaux de taille de cohorte et des contrôles de divulgation statistique destinés à empêcher la ré-identification à partir des réponses agrégées, conformément aux exigences d'anonymisation de l'art. 28 de la Loi 25 du Québec. Vous NE POUVEZ PAS être ré-identifié à partir d'une réponse d'indicateur de référence.
